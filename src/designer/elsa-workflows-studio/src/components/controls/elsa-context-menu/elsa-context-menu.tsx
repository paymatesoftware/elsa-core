...
export class ElsaContextMenu {
  ...
  render() {
    return (
      <div class="elsa elsa-relative elsa-flex elsa-justify-end elsa-items-center" ref={el => this.element = el}>
        <button onClick={() => this.toggleMenu()} aria-has-popup="true" type="button"
          ...
          <svg class="elsa elsa-w-5 elsa-h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div ref={el => this.contextMenu = el}
             ...
             class="elsa hidden elsa-z-10 elsa-mx-3 elsa-origin-top-right elsa-absolute elsa-right-7 elsa-top-0 elsa-w-48 elsa-mt-1 elsa-rounded-md elsa-shadow-lg">
          <div class="elsa elsa-rounded-md elsa-bg-white elsa-shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="project-options-menu-0">
            {this.menuItems.map(menuItem => {
              const anchorUrl = menuItem.anchorUrl || '#';
              return (
                <div class="elsa elsa-py-1">
                  <a href={anchorUrl}
                     onClick={e => this.onMenuItemClick(e, menuItem)}
                     class="elsa elsa-group elsa-flex elsa-items-center elsa-px-4 elsa-py-2 elsa-text-sm elsa-leading-5 elsa-text-gray-700 hover:elsa-bg-gray-100 hover:elsa-text-gray-900 focus:elsa-outline-none focus:elsa-bg-gray-100 focus:elsa-text-gray-900"
                     role="menuitem">
                    {menuItem.icon ? <span class="elsa elsa-mr-3 ">{menuItem.icon}</span> : undefined}
                    {menuItem.text}
                  </a>
                </div>
              );
