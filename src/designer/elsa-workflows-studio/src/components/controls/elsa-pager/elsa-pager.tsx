...
export class ElsaPager {
  ...
  render() {
    ...
    const renderNextButton = function () {
      ...
      return <a href={`/${getNavUrl(page + 1)}`}
                onClick={e => self.onNavigateClick(e, page + 1)}
                class="elsa elsa-ml-3 elsa-relative elsa-inline-flex elsa-items-center elsa-px-4 elsa-py-2 elsa-border elsa-border-gray-300 elsa-text-sm elsa-leading-5 elsa-font-medium elsa-rounded-md elsa-text-gray-700 elsa-bg-white hover:elsa-text-gray-500 focus:elsa-outline-none focus:elsa-shadow-outline-blue focus:elsa-border-blue-300 active:elsa-bg-gray-100 active:elsa-text-gray-700 elsa-transition elsa-ease-in-out elsa-duration-150">
        {t('Next')}
      </a>
    }
    ...
    const renderChevronLeft = function () {
      ...
      return (
        <a href={`${getNavUrl(page - 1)}`}
           ...
           aria-label="Previous">
          <svg class="elsa elsa-h-5 elsa-w-5" x-description="Heroicon name: chevron-left" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
    ...
    const renderPagerButtons = function () {
      ...
      for (let i = fromPage; i < toPage; i++) {
        ...
        if (isCurrent) {
          buttons.push(<span
            class={`elsa -elsa-ml-px elsa-relative elsa-inline-flex elsa-items-center elsa-px-4 elsa-py-2 elsa-border elsa-border-gray-300 elsa-text-sm elsa-leading-5 elsa-font-medium elsa-bg-blue-600 elsa-text-white ${leftRoundedClass} ${rightRoundedClass}`}>
                        {i + 1}
                    </span>);
        } else {
    ...
    return (
      <div class="elsa elsa-bg-white elsa-px-4 elsa-py-3 elsa-flex elsa-items-center elsa-justify-between elsa-border-t elsa-border-gray-200 sm:elsa-px-6">
        <div class="elsa elsa-flex-1 elsa-flex elsa-justify-between sm:elsa-hidden">
          {renderPreviousButton()}
          {renderNextButton()}
        </div>
        <div class="elsa hidden sm:elsa-flex-1 sm:elsa-flex sm:elsa-items-center sm:elsa-justify-between">
          <div>
            <p class="elsa elsa-text-sm elsa-leading-5 elsa-text-gray-700 elsa-space-x-0-5">
              <span>{t('From')}</span>
              <span class="elsa elsa-font-medium">{from}</span>
              <span>{t('To')}</span>
              <span class="elsa elsa-font-medium">{to}</span>
              <span>{t('Of')}</span>
              <span class="elsa elsa-font-medium">{totalCount}</span>
              <span>{t('Results')}</span>
            </p>
          </div>
          <div>
            <nav class="elsa elsa-relative elsa-z-0 elsa-inline-flex elsa-shadow-sm">
              {renderChevronLeft()}
              {renderPagerButtons()}
              {renderChevronRight()}
            </nav>
